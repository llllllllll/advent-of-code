...
auto solution(R&& input) {
    ...
    constexpr size_t required_free = 30000000;
    const size_t used = total_size - root.size();
    const size_t min_size = required_free - used;

    size_t min = root.size();
    for (Directory const& directory : all_directories) {
        if (directory.size() >= min_size and directory.size() < min) {
            min = directory.size();
        }
    }
    return min;
