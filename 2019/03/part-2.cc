...
int main() {
    ...
    int smallest_timing = std::numeric_limits<int>::max();
    ...
    for (const auto& seg_1 : line_1) {
        ...
        for (const auto& seg_2 : line_2) {
            ...
            if (cross && cross->mdist() > 0) {
                auto s1_partial = std::abs((seg_1.start() - *cross).size());
                auto s2_partial = std::abs((seg_2.start() - *cross).size());

                int timing = line_1_timing + line_2_timing + s1_partial + s2_partial;
                if (timing < smallest_timing) {
                    smallest_timing = timing;
                }
            }

            line_2_timing += seg_2.size();
        }

        line_1_timing += seg_1.size();
    }
    std::cout << smallest_timing << '\n';
}
