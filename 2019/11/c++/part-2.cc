...
std::string render(std::string_view prog) {
    robot r(prog);

    std::unordered_map<std::tuple<int, int>, color> tile_color;
    std::unordered_set<std::tuple<int, int>> painted;

    std::tuple<int, int> x_bounds = {0, 0};
    std::tuple<int, int> y_bounds = {0, 0};

    std::tuple<int, int> coord = {0, 0};
    tile_color[coord] = color::white;
    while (true) {
        auto [p, color, more] = r.step(tile_color[coord]);
        painted.emplace(coord);
        tile_color[coord] = color;
        coord = p;
        x_bounds = {std::min(std::get<0>(x_bounds), std::get<0>(coord)),
                    std::max(std::get<1>(x_bounds), std::get<0>(coord))};
        y_bounds = {std::min(std::get<0>(y_bounds), std::get<1>(coord)),
                    std::max(std::get<1>(y_bounds), std::get<1>(coord))};
        if (!more) {
            break;
        }
    }
    ...
    for (int y = std::get<0>(y_bounds); y <= std::get<1>(y_bounds); ++y) {
        for (int x = std::get<0>(x_bounds); x <= std::get<1>(x_bounds); ++x) {
            char c = ' ';
            auto search = tile_color.find({x, y});
            if (search != tile_color.end() && search->second == color::white) {
                c = '#';
            }
            out.push_back(c);
        }
