...
auto solution(R&& input) {
    ...
    std::vector<std::reference_wrapper<board>> cands;
    for (auto& b : st.boards) {
        cands.push_back(b);
    }

    std::vector<std::reference_wrapper<board>> new_cands;
    for (int draw : st.draws) {
        for (board& b : cands) {
            b.set(draw);
            if (b.winner()) {
                winning_scores.push_back(b.score() * draw);
            }
            else {
                new_cands.push_back(b);
            }
        }
        cands = new_cands;
        new_cands.clear();
    }
    return winning_scores.back();
}
